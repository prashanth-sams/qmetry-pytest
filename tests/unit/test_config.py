    ...
    with patch('qmetry_pytest.plugin.QMetryApi') as MockQMetryApi:
        ...
        with patch('xml.etree.ElementTree.parse') as mock_parse:
            ...
            mock_root.findall.return_value = [
                MagicMock(get=MagicMock(side_effect=lambda x: 'testcase1' if x == 'name' else 'TestFeature')),
                MagicMock(get=MagicMock(side_effect=lambda x: 'testcase2' if x == 'name' else 'TestFeature'))
            ]
