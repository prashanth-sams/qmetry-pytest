...
def test_validate_qmetry_config_invalid():
    with patch('builtins.open', mock_open(read_data='qmetry.enabled=true\nopenapi.qmetry.enabled=true\nqmetry.automation.enabled=true')):
        with patch('os.path.exists', return_value=True):
            ...
            assert error_message == "openapi.qmetry.enabled and qmetry.automation.enabled cannot be both true or both false"
...
def test_create_test_execution():
    ...
    plugin._automation_import_result = MagicMock(return_value='http://example.com/upload')
